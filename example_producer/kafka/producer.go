...
type IProducerInstance interface {
	SendMessage(mi *confluent_kafka.MessageInput) (*Output, error)
	PrepareMessage(mi *confluent_kafka.MessageInput) *kafka.Message
	Close() error
}
...
func (p *producerInstance) SendMessage(mi *confluent_kafka.MessageInput) (*Output, error) {
	//TODO implement me
	panic("implement me")
}

func (p *producerInstance) PrepareMessage(mi *confluent_kafka.MessageInput) *kafka.Message {
	if mi.Partition == 0 {
